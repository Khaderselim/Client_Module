...
def create_weighted_embeddings(df, model, name_weight, brand_weight, component_weight, description_weight, price_weight):
    ...
    name_embeddings = model.encode(cleaned_names.tolist())
    ram_embeddings = model.encode(df['RAM'].str.lower().tolist())
    cpu_embeddings = model.encode(df['CPU'].str.lower().tolist())
    gpu_embeddings = model.encode(df['GPU'].str.lower().tolist())
    screen_embeddings = model.encode(df['SCREEN'].str.lower().tolist())
    description_embeddings = model.encode(cleaned_descriptions.tolist())
    price_embeddings = model.encode(cleaned_prices.tolist())
    brand_embeddings = model.encode(cleaned_brands.tolist())
